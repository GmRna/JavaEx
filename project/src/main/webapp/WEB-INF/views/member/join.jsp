...
<script>	
	function goSave() {
		...
		$.ajax({
			...
			, async : false // 비동기를 동기로 변하게 만들어줌
			, success : function(res) {
				if (res == 'true') {
					alert('중복된 이메일 입니다. 다른 이메일을 입력해주십시오.');
					$("#email").val('');
					$("#email").focus();
					isCon = false; 
					// return -> 리턴하면 ajax내부에서 멈춤 따라서 아래의 if문 실
				} 
			}
		});	
		if (!isCon) {
		...
		if(!reg_pwd.test($("#pwd").val())) {
			alert('비밀번호 : 영문, 숫자, 특수문자 조합으로 8자 이상 입력하세요.');
			return;
		}
		
		if($("#pwd").val() != $("#pwd_check").val()) {
			alert('비밀번호 확인 값이 같지 않습니다.');
			$("#pwd_check").focus();
			return;
		}
            ...
            <div class="size">
                ...
                <table class="board_write">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th>*비밀번호</th>
                            <td><input type="password" name="pwd" id="pwd" style="float:left;" autocomplete="off"> <span class="ptxt">비밀번호는 숫자, 영문, 특수문자 조합으로 8자 이상으로 입력해주세요.</span> </td>
                        </tr>
                        <tr>
                            <th>*비밀번호<span>확인</span></th>
                            <td><input type="password" name="pwd_check" id="pwd_check" style="float:left;" autocomplete="off"></td>
                        </tr>
                        <tr>
                            <th>*이름</th>
                            <td><input type="text" name="name" id="name" style="float:left;" maxlength="10"> </td>
                        </tr>
                        <tr>
                            <th>*성별</th>
                            <td>
                        ...
                        <tr>
                            <td>
                            	<input type="text" name="addr1" id="addr1" style="wideth:80%" readonly>
                            </td>
                        </tr>
                        <tr>
                            <td>
                            	<input type="text" name="addr2" id="addr2" style="wideth:80%">
                            </td>
                        </tr>
